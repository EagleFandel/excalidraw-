...
| E. Rate limiting baseline | ✅ Done | Migrated to `@nestjs/throttler` + proxy-aware global guard, with stricter auth throttling via `@Throttle`. |
| E. OpenAPI docs endpoint | ✅ Done | Migrated to `@nestjs/swagger` (`SwaggerModule`) for `/api/docs` and `/api/docs-json`. |
...
### 8.4.1 Runtime link-off check

- `rg -n "plus\.excalidraw\.com|app\.excalidraw\.com|excalidraw-redirect" excalidraw-app packages --glob "*.ts" --glob "*.tsx" --glob "*.html"`
- `rg -n "ExportToExcalidrawPlus|ExcalidrawPlusIframeExport|VITE_APP_PLUS" excalidraw-app packages`
...
- Backend build: ✅ pass
- Backend API E2E checklist (extended): ⚠️ blocked in this environment (local PostgreSQL engine unavailable for script startup)
- Frontend production build: ⚠️ blocked by pre-existing upstream TypeScript typing errors in shared collab/encryption modules (outside this change set)
- Full monorepo vitest: ⚠️ one flaky timeout observed (`packages/element/tests/zindex.test.tsx`), isolated re-run passed.


### 8.6 2026-02-11 strict alignment completion

- Official Plus/App runtime redirects are removed from active app code paths.
- Team member management UI is upgraded to drawer-style experience with locale-based copy and a11y labels.
- Offline pending-op replay now uses exponential backoff + jitter and timer scheduling via `nextRetryAt`.
- Backend docs are now generated via `@nestjs/swagger`; manual docs controller is removed.
- Backend rate limiting now uses `@nestjs/throttler`; custom in-memory guard is removed.
- `429` responses are normalized to `{ error: { code: "RATE_LIMITED", message: "Too many requests" } }`.
